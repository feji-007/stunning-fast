...
本指南覆盖桌面客户端安装与服务后台本地启动。生产部署见 [部署运维](/guide/deploy)。
...
::: tip 无需手动建表
服务启动时 `schema.ts` 会执行 `CREATE TABLE IF NOT EXISTS`（幂等），并在表为空时插入种子数据（供应商 / 模型 / 功能入口 / 视频参数 / 管理员账号）。已有数据不会被覆盖。
:::
...
::: tip 管理后台未构建？
访问 `/admin` 提示「管理后台未构建」时，先执行 `cd server && npm run admin:build`。
:::
...
| GET/POST | `/api/tasks` | 是 | 视频生成任务记录（客户端写入 / 管理员查看统计） |
